...
# vertical justification (bits 2-3) is a VT4+ feature and those bits are
# reserved on VT3 and older, so this default has to stay the VT3-safe
# horizontal-middle/vertical-top (0b0001). The firmware patches designator
# justify bytes up to JUSTIFY_CENTERED_VT4 at runtime once it knows the
# connected VT reports version 4 or later (see main.cpp), using the offsets
# recorded below.
JUSTIFY_CENTERED = 0b0001
JUSTIFY_CENTERED_VT4 = 0b0101

# Byte offset (within the pool) of each designator OutputString's justify
# field, filled in as objects are built below - consumed by the generated
# aux_n_pool_justify.h so the firmware doesn't have to hand-maintain them.
JUSTIFY_BYTE_OFFSETS = []

# Offset of the justify field within an OutputString object's own bytes, per
# the header layout in build_output_string() below: ID(2) type(1) width(2)
# height(2) bg(1) font(2) options(1) varRef(2) justify(1) ...
OUTPUT_STRING_JUSTIFY_OFFSET = 13
...
def build_working_set():
    ...
    #
    # The object list here is NOT the active mask link (that's the separate
    # activeMask attribute above) - per ISO 11783-6 it may only contain
    # output field, output shape, output graphic, picture graphic, object
    # pointer or container objects (used e.g. as this working set's own
    # designator/icon on a multi-working-set VT). A Data Mask is none of
    # those, so it can't be listed here even though it's fine as the active
    # mask; some VTs accept it anyway, but that just makes the defect latent
    # rather than valid. Reuse TEST_STRING (an OutputString, a legal type)
    # instead of adding a dedicated object.
    ...
    data += u8(1)              # 1 child: the designator object
    data += u8(0)              # 0 macros
    data += u8(0)              # 0 languages
    # child: (objectID, x, y)
    data += u16(TEST_STRING) + i16(0) + i16(0)
    return data
...
def main():
    ...
    for oid, did, label in zip(AUX_ANALOG_IDS, DESIGNATOR_ANALOG_IDS, ANALOG_LABELS):
        JUSTIFY_BYTE_OFFSETS.append(len(pool) + OUTPUT_STRING_JUSTIFY_OFFSET)
    ...
    # Sanity-check the recorded offsets actually land on the byte we think
    # they do, so a future layout change to build_output_string() fails loud
    # here instead of silently corrupting an unrelated byte at runtime.
    for offset in JUSTIFY_BYTE_OFFSETS:
        assert pool[offset] == JUSTIFY_CENTERED, (
            f"offset {offset} does not point at a justify byte "
            f"(found 0x{pool[offset]:02X}, expected 0x{JUSTIFY_CENTERED:02X})"
        )

    # Consumed by main.cpp to patch designator justification up to
    # JUSTIFY_CENTERED_VT4 at runtime, once it knows the connected VT
    # reports version 4 or later - see the comment on JUSTIFY_CENTERED above.
    justify_header_path = OUT_DIR / "aux_n_pool_justify.h"
    justify_lines = [
        "// Auto-generated by tools/build_aux_pool.py - do not edit by hand.",
        "#pragma once",
        "",
        "#include <stdint.h>",
        "",
        f"static const uint8_t AUX_N_POOL_JUSTIFY_VT3_DEFAULT = 0x{JUSTIFY_CENTERED:02X};",
        f"static const uint8_t AUX_N_POOL_JUSTIFY_VT4_ENHANCED = 0x{JUSTIFY_CENTERED_VT4:02X};",
        f"static const uint32_t AUX_N_POOL_JUSTIFY_OFFSET_COUNT = {len(JUSTIFY_BYTE_OFFSETS)};",
        "static const uint32_t AUX_N_POOL_JUSTIFY_OFFSETS[AUX_N_POOL_JUSTIFY_OFFSET_COUNT] = {"
        + ", ".join(str(o) for o in JUSTIFY_BYTE_OFFSETS) + "};",
        "",
    ]
    justify_header_path.write_text("\n".join(justify_lines), encoding="ascii")

    print(f"Wrote {iop_path} ({len(pool)} bytes)")
    print(f"Wrote {header_path}")
    print(f"Wrote {data_header_path}")
    print(f"Wrote {justify_header_path}")
